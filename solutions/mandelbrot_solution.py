...
def mandelbrot(image, min_x, max_x, min_y, max_y, max_iters, escape_radius2):
    ...
    c_r = min_x + (x / width) * (max_x - min_x)
    c_i = min_y + (y / height) * (max_y - min_y)
...
blocks_per_grid_x = (image_gpu.shape[1] + threads_per_block[0] - 1) // threads_per_block[0]
blocks_per_grid_y = (image_gpu.shape[0] + threads_per_block[1] - 1) // threads_per_block[1]
